...
public class OdsEmitter extends AbstractOdfEmitter
{
	...
	protected static Logger logger = Logger.getLogger( OdsEmitter.class
			.getName( ) );
	...
	protected OdsLayoutEngine createLayoutEngine( OdsContext context,
			OdsEmitter emitter )
	{
	...
	private void setupRenderOptions()
	{
		...
		Object multipleSheet = renderOptions
				.getOption(LARGE_ODS_FORMAT);
	...
	{		
		...
		if ( list.getChildren( ) == null )
		{
			...
			engine.addData( OdsLayoutEngine.EMPTY, list.getComputedStyle( ),
	...
	protected Data addDataContent( IDataContent data )
	{
		...
		{
			outputData = engine.addData( data.getText( ), style,
					url, bookmark, height );
		}
		else
		{
			...
			if ( type == SheetData.STRING )
			{
				...
				outputData = engine.addData( data.getText( ), style, url,
			...
			else if ( type == Data.NUMBER )
			{
				...
				outputData = engine.addData( data.getValue( ), style, url,
						bookmark, locale, height );
			}
			else
			{
				...
				outputData = engine.addDateTime( data, style, url, bookmark,
						locale, height );
			}
		}
		return outputData;
	...
	{		
		...
		writer.startSheet( tableStyle, colStyles,
				sheetName );
	...
	protected void outputRowData( RowData rowData ) throws IOException
	{
		...
		for ( int i = 0; i < datas.length; i++ )
		{
			...
			writer.outputData( data, data.getStyleId( ),
					start, span );
		}
		writer.endRow( );
	}
		...
		if ( linkAction != null )
		{
			...
			switch ( linkAction.getType( ) )
			{
				case IHyperlinkAction.ACTION_BOOKMARK :
					hyperlink = new HyperlinkInfo( IHyperlinkAction.ACTION_BOOKMARK,
							bookmark, tooltip );
				...
				case IHyperlinkAction.ACTION_HYPERLINK :
					...
					hyperlink = new HyperlinkInfo( IHyperlinkAction.ACTION_HYPERLINK,
							url, tooltip );
					break;
				case IHyperlinkAction.ACTION_DRILLTHROUGH :
					url = EmitterUtil.getHyperlinkUrl( linkAction,
							reportRunnable, actionHandler, reportContext );
					hyperlink = new HyperlinkInfo( IHyperlinkAction.ACTION_DRILLTHROUGH,
							url, tooltip );
					break;
			}
		}
			...
			while ( iter.hasNext( ) )
			{
				...
				if ( child instanceof ITableContent )
				{
					...
					if ( isEmbededTable( (ITableContent) child ) )
					{
						logger
								.log( Level.WARNING,
										"ODS page header and footer don't support embeded grid." );
						return false;
					}
    			}
				if ( isHtmlText( child ) )
				{
					logger
							.log( Level.WARNING,
									"ODS page header and footer don't support html text." );
					return false;
				}
			}
		}
